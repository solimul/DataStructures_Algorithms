    ...
    void DFS_traversal()
    {
        ...
        for(int i=0;i<graph.gm->num_vertices;i++)
        {
            ...
            {   cout<<"\nvisiting :"<<i+1;
                ...
                visited[i] = true;
                DFS_recursive(i); // This call will visit the connected component in which i belongs.
            }
        }
    }
};
...
class BFS
{
    ...
    void BFS_visit(int u)
    {
        ...
        while(!unvisited.empty())
        {
            ...
            for(int i=0;i<graph.gm->num_vertices;i++)
            {
                if(graph.gm->matrix[v][i] && !visited[i])
                    unvisited.push(i);
            }
        }
    ...
    void BFS_traversal()
    {
        ...
        for(int i=0;i<graph.gm->num_vertices;i++)
        {
            ...
            {   cout<<"\nvisiting :"<<i+1;
                cout<<"\nNeighbors of >>"<<i+1<<" are:";
                visited[i] = true;
                BFS_visit(i); // This call will visit the connected component in which i belongs.
            }
        }
    }
};
